import { QuartzComponent, QuartzComponentConstructor, QuartzComponentProps } from "./types"

const AIBadge: QuartzComponent = ({ fileData, displayClass }: QuartzComponentProps) => {
  const isAIAssisted = fileData.frontmatter?.["ai-assisted"] === true

  if (!isAIAssisted) {
    return null
  }

  return (
    <div class={`ai-badge ${displayClass ?? ""}`}>
      <svg 
        xmlns="http://www.w3.org/2000/svg" 
        width="14" 
        height="14" 
        viewBox="0 0 24 24" 
        fill="none" 
        stroke="currentColor" 
        stroke-width="2" 
        stroke-linecap="round" 
        stroke-linejoin="round"
      >
        <path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.937A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .962 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.582a.5.5 0 0 1 0 .962L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.962 0z"/>
        <path d="M20 3v4"/><path d="M22 5h-4"/>
        <path d="M4 17v2"/><path d="M5 18H3"/>
      </svg>
      AI 辅助
    </div>
  )
}

// Quartz  CSS 注入
AIBadge.css = `
.ai-badge {
  display: inline-flex;
  align-items: center;
  gap: 0.4rem;
  padding: 0.15rem 0.75rem;
  background-color: var(--light);
  border: 1px solid var(--lightgray);
  color: var(--secondary);
  border-radius: 999px;
  font-size: 0.8rem;
  font-weight: 500;
  margin-top: 0.5rem;
  margin-bottom: 1rem;
  font-family: var(--bodyFont);
  transition: all 0.2s ease;
}

/* 鼠标悬停效果 */
.ai-badge:hover {
  background-color: var(--highlight);
  border-color: var(--secondary);
  transform: translateY(-1px);
}

.ai-badge svg {
  opacity: 0.8;
}
`

export default (() => AIBadge) satisfies QuartzComponentConstructor